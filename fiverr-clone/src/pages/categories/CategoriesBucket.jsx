            ...
            <div className='pb-10 pt-16 bg-pink-50'>
                <div className='container'>
                    <h2 className='text-3xl font-bold'>
                        {jobType.map((job) => {
                            return <i key={job.index}>{job.tenLoaiCongViec}</i>
                        })}
                    </h2>
                    <div className='grid lg:grid-cols-4 pt-6 gap-x-12 gap-y-6'>
                        {jobType.map(job1 => {
                            return job1.dsNhomChiTietLoai.map(item => {
                                return <div className='col-span-1' key={item.id}>
                                    <div>
                                        <img src={item.hinhAnh !== '' ? item.hinhAnh : "https://tse1.mm.bing.net/th?id=OIP.H9QYh9DOTV8tTJ_TH-ytrAHaFj&pid=Api&P=0"} alt='' className="object-cover w-full mb-4 sm:h-96 dark:bg-gray-500 rounded-md" style={{ height: '220px' }} />
                                        <h2 className=" font-semibold pb-2" style={{ fontSize: '23px' }}>{item.tenNhom}</h2>
                                        {item.dsChiTietLoai.map(detail => {
                                            return <a href='#detail' style={{ fontSize: '18px',display:'block' }} className=" dark:text-gray-400 cursor-pointer hover:text-pink-400 pb-2" key={detail.id} onClick={() =>
                        ...
                        return <div className="mb-6 flex flex-col max-w-lg p-6 space-y-6 overflow-hidden rounded-lg shadow-md bg-white" key={list.index} onClick={() => navigate(`/jobDetail/${list.id}`)}>
                            <div className="flex space-x-4">
                                ...
                                <div className="flex flex-col space-y-1">
                                    <a rel="noopener noreferrer" href="#" className=" font-semibold text-green-500" style={{fontSize:'16px'}}>{list.tenNguoiTao}</a>
                                    <span className="text-xs dark:text-gray-400">{Math.floor(Math.random() * 24) + 1} hours ago</span>
                                </div>
                            </div>
                            <div>
                            ...
                            <div className="flex flex-wrap justify-between">
                                ...
                                <div className="flex space-x-2 text-sm dark:text-gray-400">
                                    ...
                                    <span className='font-bold text-black'>${list.congViec.giaTien}</span>
                                </div>
                            </div>
                        </div>
                    }
